...
public class TCPListener implements Runnable {
	...
	private MyLight controllingLight; // Need this object reference to call it's
										// methods
			...
			try {
				...
				try {
					...
					try {
						...
						// 1. Get the latest R,G,B via the getter methods (if
						// FREE->USED)
						// 2. If USED->FREE init R,G,B with "low light" values
						// 3. Call the setRGB function with the RGB values
						BufferedReader inFromClient = new BufferedReader(
								new InputStreamReader(socket.getInputStream()));
						sensorStatus = inFromClient.readLine();
						LOG.info("Received " + sensorStatus + " from MQTT broker");
						controllingLight.MQTTHandler(sensorStatus);
						//socket.close();
